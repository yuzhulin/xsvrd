...
USING_CONFFILE_NAMESPACE

extern "C" 
{
	IConfigFile* CreateConfigFile(const char* pszFileName)
	{
		CConfigFile* pConfigFile = new CConfigFile(pszFileName);
		return pConfigFile;
	}
}
...
CConfigFile::CConfigFile(const char* pszFileName)
{
    m_pszFilename = NULL;
    m_pszContent = NULL;
    m_pszShadow = NULL;
...
/*************************************************
  ...
  Others:
*************************************************/
bool CConfigFile::OpenFile(const char *pszFilename)
{
    FILE    *fp;
    size_t  len;
    int     result;
...
bool CConfigFile::GetItemValueShort( const char *pszSectionName, 
                    const char *pszKeyName, 
                    short &shReturnedValue,
					short shDefaultValue)
{
	unsigned int uiRet = GetItemValue( pszSectionName, 
									pszKeyName, 
									shReturnedValue );
	if( uiRet == 0 )
	{
        shReturnedValue = shDefaultValue;
	}
	return true;
}

bool CConfigFile::GetItemValueInt( const char *pszSectionName,
                                   const char *pszKeyName,
                                   int &lReturnedValue,
                                   int lDefaultValue)
{
...
bool CConfigFile::GetItemValueString(const char *pszSectionName,
                                  const char *pszKeyName,
